...
import { Crown, Mail, Package, Shield, Sparkles, ChevronRight, Loader2 } from 'lucide-react';
import Link from 'next/link';
import Image from 'next/image';

const ProfileClient = ({ initialUser }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const planRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/users/${initialUser.id}/plan`, {
          headers: { 
  ...
  if (!user) {
    return <div className="p-8 text-center text-[#115a88]">Please login first.</div>;
  }

  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Loader2 className="w-8 h-8 text-[#115a88] animate-spin" />
      </div>
    );
  }
