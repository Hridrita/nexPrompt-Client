...
import { z } from "zod";
import { motion } from "framer-motion";
import { UploadCloud, X, Loader2, Clock, LayersPlus, Infinity, Crown } from "lucide-react";
import { createPrompt } from "@/lib/action/prompts";
import toast from "react-hot-toast";
import { authClient } from "@/lib/auth-client";
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-medium ${
              isPremium ? "text-amber-700" : "text-gray-500"
            }`}>
              {isPremium ? "Premium Plan:" : "Free Plan:"}
            </span>
            <span className={`text-xs font-semibold ${
              isPremium ? "text-amber-800" : "text-gray-600"
            }`}>
              {isPremium 
                ? "Unlimited Prompts" 
                : `${promptCount} / 3 prompts used`
              }
            </span>
          </div>
          {!isPremium && promptCount > 0 && (
          ...
          {!isPremium && (
            <a href="/payment" className="text-xs font-medium text-[#0a9fd4] hover:underline">
              Upgrade →
            </a>
          )}
        </motion.div>

        {/* বাকি ফর্ম ফিল্ড আগের মতো */}
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <label className={labelClass}>Prompt Title</label>
          <input {...register("title")} placeholder="e.g. Ultimate SEO Blog Generator" className={inputClass} />
          {errors.title && <p className={errorClass}>{errors.title.message}</p>}
