    ...
    <div className="w-full">
      ...
      {prompts.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-300 py-16 text-center">
          <p className="text-zinc-500 text-sm">No prompts yet. Create one to see it here.</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-[#f3f7fb] shadow-sm">
          <table className="min-w-full divide-y divide-zinc-200">
            <thead className="bg-zinc-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wide">Prompt</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wide">Visibility</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wide">Copies</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {prompts.map((p) => {
                ...
                return (
                  <tr key={id} className={`hover:bg-zinc-50/70 transition-colors ${isBusy ? "opacity-50 pointer-events-none" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        <button onClick={() => setEditingPrompt(p)} className="p-2 rounded-lg text-zinc-500 hover:bg-[#e6f4fb] hover:text-[#0a6c9b] transition-colors" title="Edit">
                          <Pencil width={16} height={16} />
                        </button>
                        <button onClick={() => setDeleteTarget(p)} className="p-2 rounded-lg text-zinc-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="Delete">
        ...
        {deleteTarget && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <p className="mt-1.5 text-sm text-zinc-500">"{deleteTarget.title}" will be permanently removed. This can't be undone.</p>
              <div className="mt-5 flex justify-end gap-2">
                <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 text-sm font-medium text-zinc-600 rounded-lg hover:bg-zinc-100 transition-colors">Cancel</button>
        ...
        {editingPrompt && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 10 }}
              onClick={(e) => e.stopPropagation()} className="w-full max-w-lg rounded-2xl bg-white shadow-xl max-h-[85vh] overflow-y-auto">
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 bg-gradient-to-r from-[#115a88] to-[#0a9fd4] sticky top-0 rounded-t-2xl">
                ...
                <button onClick={() => setEditingPrompt(null)} className="p-1.5 rounded-lg text-white/80 hover:bg-white/15 hover:text-white transition-colors">
                  <Xmark width={18} height={18} />
                </button>
              </div>
              <EditPromptForm prompt={editingPrompt} onCancel={() => setEditingPrompt(null)} onSubmit={handleUpdateSubmit} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
...
function EditPromptForm({ prompt, onCancel, onSubmit }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5 bg-gradient-to-b from-[#f3f9fc] to-white">
      {/* thumbnail preview + title */}
      <div className="flex gap-4 items-start">
        {prompt.thumbnail && (
          <div className="relative h-16 w-16 rounded-xl overflow-hidden bg-zinc-100 shrink-0 ring-2 ring-[#0a9fd4]/20">
            <Image src={prompt.thumbnail} alt={form.title} fill className="object-cover" />
          </div>
        )}
      ...
      <div className="flex justify-end gap-2 pt-3 border-t border-[#d6ebf4]">
        ...
        >
          {submitting && <span className="h-3.5 w-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />}
